...
func writeFile(param PostFileParam) error {
	...
	w_filepath, err := StringToWCharPtr(filepath)
	if err != nil {
	...
	var modifiedTime syscall.Filetime
	modifiedTime.LowDateTime = (uint32)(param.lastModified & 0xffff)
	modifiedTime.HighDateTime = (uint32)((param.lastModified >> 16) & 0xffff)
